...
class ListEmployee extends Component {
  ...
  componentDidMount() {
    ...
    if (user) {
      this.setState({
        currentUser: user.businessName,
        currentUser: user.branch,
      });
      console.log("test__________API____________________AUTH", AuthService.listEmployee('burinLKB', 'LKB'));
      console.log("test__________API____________________USER", UserService.listQueue('employee'));
  ...
  render() {
    if (this.state.redirect) {
      return (<Redirect
        to={{
          pathname: "/profile",
          state: { business_name: this.state.businessName, username: this.state.username, branch: this.state.branch }
        }}
      />)
    }
    var border = {
    ...
    console.log("listEmployee====================", this.state.listEmployee)
        ...
        {this.state.listEmployee.length > 0 ? (
          <div>
            ...
            <Container style={border2} className="text-center">
              {this.state.listEmployee.map((item, i) => {
                return (
                  <Row sm md className="text-center" style={{ padding: "10px", }, border} key={i}>
                    ...
                    <Col sm md style={{ borderRight: "2px solid rgba(0,0,0,0.46)", padding: "15px" }}>
                    <button type="button" className="btn btn-info btn-lg" onClick={() => this.handleRedirect(item.username, item.business_name)} >Detail</button>
                    </Col>

                    <Col sm md style={{ borderRight: "2px solid rgba(0,0,0,0.46)", padding: "15px" }}>
                    <button type="button" className="btn btn-info btn-lg" onClick={this.handleEdit} >Edit</button>
                    </Col>

                    <Col sm md style={{ borderRight: "2px solid rgba(0,0,0,0.46)", padding: "15px" }}>
                    <button type="button" className="btn btn-info btn-lg" onClick={() => this.handleRedirect(item.username, item.business_name)} >Delete</button>
                    </Col>
